    ...
    <template #body>
      <div v-if="error && !stats" class="p-12 text-center">
        <div class="p-8 rounded-3xl bg-error-500/10 border border-error-500/20 max-w-md mx-auto">
          ...
          <h3 class="text-lg font-bold dark:text-white mb-2">Gagal Memuat Statistik</h3>
      ...
      <div v-else-if="stats" class="space-y-8 pb-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="flex flex-col gap-4">
              <div class="flex items-center justify-between">
                <div class="p-3 bg-white/20 rounded-2xl">
                  <UIcon name="i-lucide-users" class="w-6 h-6 dark:text-white" />
                </div>
                <div class="flex flex-col items-end">
                  <span class="text-[10px] font-black dark:text-white/60 uppercase tracking-widest">Growth (30d)</span>
                  <span class="text-xs font-bold dark:text-white flex items-center gap-1">
              ...
              <div>
                ...
                <h3 class="text-4xl font-black dark:text-white italic tracking-tighter">{{ stats.overview?.users?.total || 0 }}</h3>
                <p class="text-[10px] dark:text-white/50 mt-1 uppercase font-bold tracking-widest">{{ stats.overview?.users?.verified || 0 }} Verified</p>
              </div>
            </div>
          </UCard>

          <!-- Tournament Stats -->
          <UCard class="dark:bg-neutral-900 border border-neutral-200 dark:border-white/5 hover:border-indigo-500/30 transition-all duration-300 group">
            <div class="flex flex-col gap-4">
              <div class="flex items-center justify-between">
              ...
              <div>
                <p class="dark:text-neutral-400 text-sm font-bold tracking-tight">Total Turnamen</p>
                <h3 class="text-4xl font-black dark:text-white italic tracking-tighter">{{ stats.overview?.tournaments?.total || 0 }}</h3>
          ...
          <!-- Team Stats -->
          <UCard class="dark:bg-neutral-900 border border-neutral-200 dark:border-white/5 hover:border-emerald-500/30 transition-all duration-300 group">
            <div class="flex flex-col gap-4">
              <div class="flex items-center justify-between">
              ...
              <div>
                <p class="dark:text-neutral-400 text-sm font-bold tracking-tight">Total Tim</p>
                <h3 class="text-4xl font-black dark:text-white italic tracking-tighter">{{ stats.overview?.teams?.total || 0 }}</h3>
          ...
          <!-- Platform Status -->
          <UCard class="dark:bg-neutral-900 border border-neutral-200 dark:border-white/5">
            <div class="flex flex-col h-full justify-between gap-4">
              <div class="flex items-center justify-between">
                <div class="p-3 bg-neutral-50 dark:bg-white/5 rounded-2xl">
                  <UIcon name="i-lucide-activity" class="w-6 h-6 dark:text-neutral-400" />
                </div>
              </div>
              <div>
                ...
                <div class="flex items-center gap-3">
                  ...
                  <span class="text-xl font-black dark:text-white uppercase italic tracking-widest">Optimal</span>
                </div>
              </div>
            </div>
          </UCard>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <UCard class="lg:col-span-2 dark:bg-neutral-900/40 backdrop-blur-xl border border-neutral-200 dark:border-white/5" :ui="{ header: 'border-b border-neutral-200 dark:border-white/5', body: 'p-0' }">
            <template #header>
              <div class="flex items-center justify-between px-2">
                <h3 class="text-sm font-black dark:text-white uppercase tracking-widest">Aktivitas Terkini</h3>
                <UButton label="Lihat Semua" variant="link" color="neutral" size="xs" />
              </div>
            </template>

            <div class="divide-y divide-white/5">
              <div v-for="activity in stats.activities" :key="activity.id" class="p-5 flex items-start gap-4 hover:bg-neutral-50 dark:bg-white/[0.02] transition-colors group">
                ...
                <div class="flex-1 min-w-0">
                  <p class="text-sm text-neutral-200 font-medium">
                    <span class="font-bold dark:text-white">{{ activity.user }}</span> membuat turnamen
          ...
          <!-- Sport Distribution -->
          <UCard class="dark:bg-neutral-900/40 backdrop-blur-xl border border-neutral-200 dark:border-white/5" :ui="{ header: 'border-b border-neutral-200 dark:border-white/5' }">
            <template #header>
              <h3 class="text-sm font-black dark:text-white uppercase tracking-widest">Olahraga Terpopuler</h3>
            </template>

            <div class="space-y-6">
              <div v-for="sport in stats.distribution" :key="sport.name" class="space-y-2">
                <div class="flex justify-between items-end">
                  <span class="text-xs font-bold dark:text-neutral-300 uppercase tracking-tight">{{ sport.name }}</span>
                  <span class="text-[10px] font-black text-indigo-400">{{ sport.count }} Turnamen</span>
                </div>
                <div class="h-1.5 w-full bg-neutral-50 dark:bg-white/5 rounded-full overflow-hidden">
                  <div 
      ...
      <div v-else class="space-y-8">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
          <USkeleton v-for="i in 4" :key="i" class="h-32 rounded-2xl bg-neutral-50 dark:bg-white/5" />
        </div>
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <USkeleton class="lg:col-span-2 h-96 rounded-2xl bg-neutral-50 dark:bg-white/5" />
          <USkeleton class="h-96 rounded-2xl bg-neutral-50 dark:bg-white/5" />
        </div>
      </div>
    </template>
  </UDashboardPanel>
