...
class EloquentTournamentRepository implements TournamentRepositoryInterface
{
    ...
    {
        return Tournament::with(['user', 'sport', 'prizes'])->latest()->paginate($perPage);
    }
    ...
    {
        $defaultIncludes = ['user', 'sport', 'prizes'];
    ...
    {
        return Tournament::with(['sport', 'prizes'])->where('user_id', $userId)->latest()->paginate($perPage);
    }
